...
- **Components**: `position` (x, y — tile units), `needs` (hunger, thirst,
  ...
  `aiState` (the seven Utility AI base scores, persisted for the debug view
  and the determinism tests without recomputing the AI), `lineage`
  (generation, parentA, parentB), and `reproductive` (binary sex, fertility-
  scaled reproduction cooldown, cached eligibility flag).
...
selectIntents → moveAgents → interactWithResources → updateNeeds
  → updateDeaths → updateMemory → regenerateResources → updateAging
  → updateMortality → updateReproduction → tick++
```

`updateReproduction` runs last so a child is born at age 0 and only acts on
the following tick; `updateMortality` adds age pressure after aging advances
the clock, and the existing death system (running earlier, after needs) is
what actually removes agents.

`ai/` owns *decisions*; systems own *consequences*. The Utility AI
(`ai/utility-ai.ts`) scores the seven candidate actions (Rest, Wander,
SeekFood, SeekWater, Eat, Drink, SeekPartner) with bounded [0, 1] utilities,
applies deterministic tie-break noise + action hysteresis, and writes the
winning intent into the same `intent` store every other system already reads.
...
(`SNAPSHOT_FORMAT_VERSION = 3`): per frame only tick/time/population/births/
deaths/max-generation/averages/resources/trait-distributions plus SoA typed
arrays (ids, x, y, strength, intelligence, speed, intent kind) — a
`formatVersion` field allows the format to evolve (delta snapshots, binary
payloads) without ambiguity. Full per-agent data is fetched on demand via
`get-agent` (which also returns the live seven-action AI utility table,
movement target, memory lists, life stage, sex, lineage and per-gene
inheritance origins for the inspector), so per-frame cost stays O(population)
with small constants at 10 snapshots/second even for thousands of agents. The
worker rate-limits snapshots to `snapshotIntervalMs` (100 ms) and always sends
one on init/pause/resume/re-init.
...
`birth`, `mutation`, `old_age_death`. Event history is not persisted yet
(deliberate — later phase).

## 9. Persistence

`persistence/serialization.ts` defines the versioned save format
(`SAVE_FORMAT_VERSION = 3`) capturing everything needed to resume bit-for-bit:
...
memory store, third RNG stream) and Phase 3 (reproduction, lineage + a fourth
RNG stream). Round-trip and continuation equality are enforced by tests and by
...
## 11. Scope ladder (Phase 2 done, Phase 3 done — what stays out)
...
social/civilization ladder (tribes, culture, language, technology, agriculture,
economy, trade, warfare, cities, civilization). Learning stays a simple,
deterministic associative update (bounded memory values + intelligence-
modulated rates), never a neural model.
